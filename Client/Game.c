                            ...
                            rr_ui_link_toggle(
                                rr_ui_set_fill_stroke(
                                    rr_ui_h_container_init(rr_ui_container_init(), 0, 0,
                                        rr_ui_text_input_init(350, 30, &this->cache.nickname[0], 10, "_0x4346"), 
                                        NULL
                                    ), 
        ...
        case rr_clientbound_animation_update:
        {
            while (proto_bug_read_uint8(&encoder, "continue"))
            {
                ...
                case rr_animation_type_chat:
                    ...
                    struct rr_game_chat_message *message =
                        &this->chat.messages[this->chat.at];
                    proto_bug_read_string(&encoder, message->sender_name, 64,
                                          "name");
                    proto_bug_read_string(&encoder, message->message, 64,
                                          "chat");
                    sprintf(message->text, "%s: %s", message->sender_name,
                            message->message);
                    break;
                default:
                    break;
    ...
    if (this->cache.use_mouse)
    {
        ...
        proto_bug_write_float32(
            ...
            "mouse y");
               ...
               if(rr_dom_set_check_focused("_0x4523") == 0){
