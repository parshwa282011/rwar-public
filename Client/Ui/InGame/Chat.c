...
rr_ui_chat_text_init(struct rr_game_chat_message *message)
{
...
struct rr_ui_element *rr_ui_message_box_init(struct rr_game *game)
{
    return rr_ui_v_container_init(
        ...
        //   rr_ui_chat_text_init(&game->chat.messages[10])
        NULL);
}

struct rr_ui_element *rr_ui_chat_bar_init(struct rr_game *game)
{
    struct rr_ui_element *chatbar =
        rr_ui_text_input_init(200, 18, game->chat.sending, 64, "_0x4523");
    ...
    struct rr_ui_element *this = rr_ui_set_background(
        rr_ui_h_container_init(
            rr_ui_container_init(), 10, 0,
            rr_ui_choose_element_init(
                rr_ui_flex_container_init(
                    rr_ui_text_init("[Global]", 14, 0xffffffff), chatbar, 10),
                text, chatbar_choose),
            NULL),
        0x80000000);
    ...
    struct rr_ui_element *chat = rr_ui_v_container_init(
        rr_ui_container_init(), 10, 10,
        rr_ui_set_justify(rr_ui_message_box_init(game), -1, -1),
        rr_ui_set_justify(this, -1, -1), NULL);
