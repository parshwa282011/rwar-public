...
struct rr_ui_element *rr_ui_petal_tooltip_init(uint8_t id, uint8_t rarity)
{
    ...
    {
        char *extra_range = malloc((sizeof *extra_range) * 16);
        sprintf(extra_range, "%s", rr_sprintf(fmt, 100 + 75 * rarity));
        double self_damage = 0.1 * rarity;
        char *extra_self_damage = malloc((sizeof *extra_self_damage) * 16);
        sprintf(extra_self_damage, "Self Damage: %.1f/s", 25 * self_damage);
        rr_ui_container_add_element(
        this, rr_ui_set_justify(
                rr_ui_h_container_init(
                    rr_ui_container_init(), 0, 0,
                    rr_ui_text_init("Range: ", 12, 0xffbf29c2),
                    rr_ui_text_init(extra_range, 12, 0xffffffff),
                    rr_ui_text_init(extra_self_damage, 12, 0xffffffff),
                    NULL),
                -1, 0));
    }
    ...
    {
        char *extra = malloc((sizeof *extra) * 8);
        sprintf(extra, "%s", rr_sprintf(fmt, 6 * RR_PETAL_RARITY_SCALE[rarity].heal));
        rr_ui_container_add_element(
            this,
            rr_ui_set_justify(rr_ui_h_container_init(
                                  rr_ui_container_init(), 0, 0,
