                            ...
                            size_t size)
{
    ...
    {
        lws_get_peer_addresses(ws, lws_get_socket_fd(ws), ip_buffer,
                               sizeof(ip_buffer), NULL, 0);
        ...
        for (uint64_t i = 0; i < RR_MAX_CLIENT_COUNT; i++)
            if (!rr_bitset_get_bit(this->clients_in_use, i))
            {
                ...
                memcpy(this->clients[i].ip_address, ip_buffer,
                       sizeof(this->clients[i].ip_address));
        ...
        lws_close_reason(ws, LWS_CLOSE_STATUS_GOINGAWAY,
                         ...
                         sizeof "too many active clients" - 1);
        return -1;
    }
...
void rr_server_run(struct rr_server *this)
{
    ...
    }
    {
        ...
        char api_path[256];
        snprintf(api_path, sizeof(api_path), "/api/%s", getenv(RR_API_SECRET));
        client_info.path = api_path;
