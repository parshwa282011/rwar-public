                            ...
                            struct rr_component_player_info *player_info)
{
    ...
    for (uint64_t outer = 0; outer < player_info->slot_count; ++outer)
    {
     ...
     else if (data->id ==rr_petal_id_uranium)
     {
         double damage = 0.25 * slot->rarity;
         rr_component_health_set_health(health, health->health - damage);
     }                           
        ...
        for (uint64_t inner = 0; inner < slot->count; ++inner)
        {
            ...
            else
            {
                ...
                if (data->id == rr_petal_id_egg)
                {
                    ...
                    {
                        m_id = rr_mob_id_trex;
                        m_rar = petal->rarity >= 4 ? petal->rarity - 1 : (petal->rarity >= 2 ? petal->rarity - 2 : 0);
                    }
